...
export default {
	...
	methods: {
		...
		loadList (callback) {
			...
			this.dataList = []
			api.getuserupoperlist2({sysNo: this.dicType}).then(res => {
				if (res.data != null) {
					res.data.map(item => {
						let upUserCode = item.split("-")[0]
						let upUserName = item.split("-")[1]
						this.dataList.push({ value: upUserCode, label: upUserCode + '-' + upUserName })
					})
				 ...
				 console.log(this.dataList,'0999999999999987===')
				callback && callback()
			})
		},
