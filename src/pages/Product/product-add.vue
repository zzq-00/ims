  ...
  export default {
	...
	methods: {
		initProduct () {
			...
			if (!this.productId) {
				...
				productApi.getNextId().then(res => {
					this.productServiceId = res.data
					this.treeData.push({
						title: '填写产品信息',
						key: this.productServiceId,
						type: 'product',
						saveStatus: false,
						slots: {icon: 'product'},
						value: {productServiceId: this.productServiceId},
						children: []
					})
					this.selectedKeys = []
				...
				this.productServiceId = this.productId
			}
		},
		...
		queryByEdit (productCode) {
			productApi.productQueryByEdit({productCode: productCode}).then(res => {
