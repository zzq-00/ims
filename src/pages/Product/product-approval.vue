<template>
  <a-card :bordered="false">
    <a-card>
        ...
        <a-row :gutter="16">
          <a-col :span="6">
            <a-form-item label="产品">
              <ProductSelect :allowClear="true" v-decorator="['product', {initialValue: ''}]" />
            </a-form-item>
          </a-col>
          <a-col :span="6">
      ...
      <a-divider orientation="left">
        <a-icon type="audit" /> 待审批任务公共池</a-divider>
      ...
      <a-divider orientation="left">
        <a-icon type="audit" /> 待审批任务个人池</a-divider>
      ...
      <a-row :gutter="16">
        <a-col :span="24" style="text-align:right;margin:10px 0 0 0">
          <a-form-item>
            <a-button :disabled="this.selectedRowsSelf.length==0" type="primary" class="editable-add-btn" style="margin:0px 10px 0 0;" @click="goToCheck">进入</a-button>
          </a-form-item>
        </a-col>
      </a-row>
    </a-card>
...
export default {
	...
	methods: {
		...
		loadPageDataPublic () {
			...
			this.loading = true
			api.examineOrderPublicList(data).then(res => {
		...
		loadPageDataSelf () {
			...
			api.examineOrderSelfList(data).then(res => {
		...
		pick () {
			...
			api.examineOrderPick(data).then(res => {
				this.searchHandle()
				this.searchHandleSelf()
			}).finally(() => {
		...
		goToCheck () {
			...
			this.$router.push({
				name: 'ProductApprovalDetail', params: {taskCode: taskCode, busiCode: busiCode}
			})
		},
		resetFilterForm () {
