  ...
  <div style="padding:24px;text-align:left;">
    ...
    <a-row>
      ...
      <a-col :span="4" style="width:16.6%; overflow: auto; height: 500px;border:1px solid #e8e8e8;margin-top:12px;margin-bottom:12px">
        <a-tree showIcon showLine  :autoExpandParent="true" @select="onSelect" :selectedKeys="selectedKeys" :defaultSelectedKeys ="defaultSelectedKeys" :treeData="treeData">
...
export default {
	...
	methods: {
		queryByEdit () {
			productApi.productQueryByEdit({productCode: this.productCode}).then(res => {
				this.initData = res.data
				this.initTreeData(this.initData.psList)
			})
			productApi.examineTrace({productCode: this.$route.query.productCode}).then(res => {
		...
		onSelect (selectedKeys, info) {
			...
			if (type === 'product') {
				...
				this.productType = 2
				this.serviceType = '02'
				this.$refs.serviceForm.editForm(info.node.dataRef.value, 'readOnly', type)
			} else if (type === 'service2') {
				this.productType = 3
				this.serviceType = '03'
				this.$refs.serviceForm.editForm(info.node.dataRef.value, 'readOnly', type, this.traverseSearchNode(this.treeData, info.node.dataRef.parentServiceId).value.serviceName)
			} else {
				this.productType = '4'
