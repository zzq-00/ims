  ...
  <div style="padding:24px;text-align:left;">
    ...
    <a-row>
      ...
      <a-col :span="4" style="width:16.6%; overflow: auto; height: 500px;border:1px solid #e8e8e8;margin-top:12px;margin-bottom:12px">
        <a-tree showIcon showLine :defaultExpandAll="true" :autoExpandParent="true" @select="onSelect" :selectedKeys="selectedKeys" :defaultSelectedKeys ="defaultSelectedKeys" :treeData="treeData">
      ...
      <a-card style="margin-right: 12px;">
        ...
        <a-col :span="24">
          <a-form-item label="审核内容" :labelCol="{ span: 3 }">
            <a-textarea v-decorator="['taskResultDesc', {initialValue: '',rules: [{ required: true, message: '审核内容必填!' }]}]"  :rows="4"/>
          </a-form-item>
        </a-col>
  ...
  export default {
	...
	mounted () {
		...
		if (this.taskCode) {
			...
			this.$router.push({name: 'ProductExamine'})
		}
	},
	methods: {
		queryByEdit () {
				...
				this.$confirm({
					...
					onOk () {
						...
						// 更新父节点的children
							...
							productApi.examineCheck(self.submitData).then(res => {
								if (res.data) {
									self.$message.success('保存成功!')
									self.$router.push({name: 'ProductExamine'})
								} else {
