        ...
        <a-icon type="bank" /> 健管卡预警列表</span>
      	<a href="#" slot="extra">
        	<a-icon :type="iconType" @click="swithTableShow" />
				...
				<a-table :rowClassName= "(record, index) => { return record.totalCount - record.nCount <= record.warningQuantity ? 'table-color-dust' : ''}"
          :scroll="{ x: 'max-content'}" v-show="showTable" :bordered="false" :pagination="pagination" :rowSelection="rowSelection" :dataSource="pageData.data" :columns="columns" :rowKey="record => record.docDefId" :indentSize=0 :loading="loading">
					<a slot="docCode" @click="showRecordInfo(record)" slot-scope="text, record">{{(text)}}</a>
          <a slot="docName" @click="showRecordInfos(record)" slot-scope="text, record">{{(text)}}</a>
