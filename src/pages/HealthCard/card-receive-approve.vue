							...
							<a-row>
								<a-col :span="24">
									<a-form-item label="审核结论">
										<a-select v-decorator="['assessResult', {initialValue: 'Y', rules: [{ required: true, message: '审核结论不能为空!' }]}]">
...
export default {
	name: 'health-card-receive-approve',
	...
	methods: {
		...
		loadPageData () {
			...
			this.loading = true
			api.receiveCardApproveQuery(data).then(res => {
		...
		submitForm () {
			this.dataForm.validateFields((error, values) => {
				...
				if (this.approveWay === 'batch') {
					api.receiveCardApprove(param).then(res => {
						this.formModal = false
						this.showTips(res.data.resultList)
						this.loadPageData()
					}).finally(() => {
					...
					api.receiveCardAllApprove(param).then(res => {
						this.formModal = false
						this.showTips(res.data.resultList)
						this.loadPageData()
					}).finally(() => {
		...
		showTips (results) {
			...
			results.forEach((item, index) => {
				if (item.flag.trim() === "0") {
					successNum += 1
					this.$message.success("成功" + successNum + "条，失败" + errorNum + "条")
				} else if (item.flag.trim() === "1") {
					errorNum += 1
					this.$message.error("失败" + errorNum + "条，" + "失败原因" + item.warnInfo)
				}
			})
		}
