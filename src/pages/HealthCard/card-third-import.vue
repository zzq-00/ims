      ...
      <a-row :gutter="16">
        <a-col :span="14" :offset="6" style="text-align:left">
          ...
          <a-button
            ...
            :loading="uploading"
            @click="BtnhandleUpload">{{ uploading ? '上传中···' : '健管卡导入' }}</a-button>
        </a-col>
      </a-row>
    </a-card>
	...
	methods: {
		...
		BtnhandleUpload () {
			...
			api.importDocThirdParty(data).then(res => {
				this.fileList = []
				this.$message.success(res.messageInfo || '上传成功')
			}).finally(() => {
		...
		// 错误清单下载
		errorListDownload () {
			api.exportDocThirdParty().then(res => {
				if (res.status === undefined) {
					// base64内容，文件名
					this.$downloadFileByBase64(res, `健管卡导入错误清单.xls`)
				} else {
		...
		// 导入模板下载
		templateDownload () {
			api.thirtyDownload({}).then(res => {
				if (res.status === undefined) {
					// base64内容，文件名
					this.$downloadFileByBase64(res, `健管卡导入模板.xls`)
				} else {
					this.$message.error('文件下载失败,请重试')
				}
			}).finally(() => {
