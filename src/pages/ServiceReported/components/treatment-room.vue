<template>
  <a-card :bordered="false">
        ...
        <a-row :gutter="16">
          <a-col :span="8">
            <a-form-item label="报送年月">
              <a-month-picker
                v-decorator="['reportedDate',{initialValue: null, rules: [ {required: true, message: '请选择报送年月'} ]}]"
        ...
        <a-row :gutter="16">
          <a-col :span="12" :offset="11">
            <a-button type="primary" class="editable-add-btn" style="float:right;margin-right:10px;"
                      :disabled="!selectedRows.length" @click="auditRecord">审核
            </a-button>
            <a-button type="" class="editable-add-btn" style="float:right;margin-right:10px;" @click="resetFilterForm">
              重置
            </a-button>
            <a-button type="primary" class="editable-add-btn" style="float:right;margin-right:10px;"
    ...
    <a-card style="margin-top:24px;">
      ...
      <a-table v-show="showTable" :scroll="{ x: 'max-content'}" :bordered="false" :pagination="pagination"
               :dataSource="pageData.data" :columns="columns" :rowKey="record => record.recordIndex" :indentSize=0
               :loading="loading"
	...
	data () {
		return {
			...
			columns: [
				...
				{title: "医病管理", dataIndex: "diseaseControl"},
