<template>
  <div>
    <a-modal v-model="formModal" style="top: 30px;" :width="modalWidth" title="服务机构/预约医院查询">
      <a-card>
        <a-form :form="filterForm" :labelCol="filterFormLayout.labelCol" :wrapperCol="filterFormLayout.wrapperCol">
          <a-row :gutter="16">
            <a-col :span="6">
             <a-form-item label="机构类型">
									<DicSelect dicType="SUPPLIER_SUB_TYPE" v-decorator="[
								'',
								{initialValue: '',rules: [{ required: true, message: '不能为空!' }]}
								]" @change="OrganizationType"></DicSelect>
...
export default {
	...
	components: { AddressSelect ,DicSelect },
	data () {
		return {
			// 查询条件
			filterFormLayout: {
	...
	methods: {
		...
		loadPageData () {
			this.filterForm.validateFields((error, values) => {
				...
				let data = {
					...
					orgType: this.OrganizationTypeSize
				}
		...
		OrganizationType(value,opetion){
			console.log(value,opetion,"123456789")
			this.OrganizationTypeSize=value
			this.loadPageData()
		},
