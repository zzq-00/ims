        ...
        <a-row :gutter="16">
          <a-col :span="6">
            <a-form-item label="报送年月">
              <a-month-picker v-decorator="['reportedDate',{rules: [ {required: true, message: '请选择报送年月'} ]}]" format="YYYY-MM" valueFormat="YYYY-MM" />
            </a-form-item>
          </a-col>
          <a-col :span="6">
            <a-form-item label="管理机构">
							<OrgSelect2 dicType="orgCode_4" :disabled="disabledOrgCode" v-decorator="['orgCode',{initialValue: null} ]" :allowClear="true" @change="searchHandle" placeholder="请选择管理机构" />
						</a-form-item>
          </a-col>
					<a-col :span="6">
...
<!--        <span slot="textVal" slot-scope="text, record">{{text || '-'}}</span>-->
        <template slot="operation" slot-scope="text, record">
          <span v-if="record.status == '03'" style="color:#1890ff;cursor:pointer" @click="deleteRowData(record)">删除</span>
			...
			<a-row :gutter="16">
        <a-col :span="24">
          <a-button type="primary" @click="jump2BranchReport" class="editable-add-btn" style="float:right;margin:10px 0 0 0">添加健管中心报送</a-button>
        </a-col>
      </a-row>
    </a-card>
		...
		exportRecord () {
			...
			this.filterForm.validateFields((error, values) => {
				...
				api.exportBranchWork(query).then(res => {
					if (res !== (undefined || '')) {
						this.$downloadFileByBase64(res, `健管中心服务人次信息.xlsx`)
					} else {
