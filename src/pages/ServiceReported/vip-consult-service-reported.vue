<template>
  <a-card :bordered="false">
    ...
    <a-card>
      <span href="#" slot="title" style="float:left;"><a-icon type="profile" /> VIP诊疗室服务量报送列表</span>
    ...
    <a-card style="margin-top:24px;">
      ...
      <a-form :form="dataForm" :labelCol="dataFormLayout.labelCol" :wrapperCol="dataFormLayout.wrapperCol">
        <a-row :gutter="16">
          ...
          <a-col :span="12">
            <a-form-item label="医院名称" :labelCol="{span: 5}">
              <a-input-search :readonly="true" v-decorator="[ 'hospitalName', {initialValue: '', rules: [ {required: true, message: '请选择医院'} ]}]" @search="onSearchHealthCenter" enterButton="搜索"/>
            </a-form-item>
          </a-col>
        </a-row>
        <a-row :gutter="16">
...
export default {
	name: "serve-apply-vip-consult-service",
	...
	data () {
		return {
			...
			columns: [
				...
				{ title: "VIP诊疗室名称", dataIndex: "hospitalName" },
	...
	methods: {
		...
		loadPageData () {
			...
			this.loading = true
			api.queryVipConsultServiceReported(data).then(res => {
		...
		onSearchHealthCenter () {
			...
			this.$refs.modalHealtheCenterSelect.show2()
		},
		onSelectItem (obj) {
			this.healthCenterOrHospitalCode = obj.serverCode
			this.dataForm.setFieldsValue({hospitalName: obj.serverName})
		},
		swithTableShow () {
			...
			this.$confirm({
				...
				onOk () {
					return new Promise((resolve, reject) => {
						...
						api.deleteVipConsultServiceReported(data).then(res => {
		...
		submitSelected () {
			...
			api.submitVipConsultServiceReported(data).then(res => {
		...
		verifyForm (formData) {
			...
			let self = this
			return new Promise((resolve, reject) => {
				api.verifyVipConsultServiceReported(data).then(res => {
					if (res.data.flag) {
						resolve()
					} else {
						if (res.data.message) {
		...
		save () {
			...
			this.dataForm.validateFields((error, values) => {
				...
				data.reportedDate = moment(data.reportedDate).format('YYYY-MM')
				this.verifyForm(values).then(() => {
					api.saveVipConsultServiceReported(data).then(res => {
						self.$message.success('保存成功')
						self.resetDataForm()
						self.loadPageData()
					})
		...
		saveAndSubmit () {
			this.dataForm.validateFields((error, values) => {
				...
				console.log(data, " ------------====================")
				this.verifyForm(values).then(() => {
					api.saveAndSubmitVipConsultCenterServiceReported(data).then(res => {
		...
		submitForm () {
			this.dataForm1.validateFields((error, values) => {
				...
				if (this.saveAndSubmitFlag === '0') {
					let data = {
						serviceTimes: JSON.stringify({'timesSourece': '04'}),
						type: '02'
					}
					data.serviceTimesBoList = this.selectedRows
