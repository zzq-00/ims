      ...
      <a-row>
        ...
        <a-col :span="6">
          <a-form-item label="陪诊人电话">
            <a-input :disabled="!status" v-decorator="['staffTel', {initialValue:'',rules: [{ pattern:/^1[3|4|5|6|7|8|9][0-9]\d{8}$/, message: '陪诊人电话格式不正确!' }]}]" />
          </a-form-item>
        </a-col>
      </a-row>
      <a-row>
        ...
        <a-col :span="6">
          <a-form-item label="PICC陪诊人电话">
            <a-input-search :disabled="!status" v-decorator="['piccStaffMobile', {initialValue:'',rules: [{ pattern:/^1[3|4|5|6|7|8|9][0-9]\d{8}$/, message: 'PICC陪诊人电话格式不正确!' }]}]" enter-button @search="selectPiccStaff" />
          </a-form-item>
        </a-col>
      </a-row>
      <a-row>
...
export default {
	...
	methods: {
		moment,
		initLoad () {
			console.log('04页面数据', this.taskInfo)
			this.status = parseInt(this.taskInfo.status) === 4// 判断是否已过了该步骤
			if (parseInt(this.taskInfo.status) >= 4) {
				let remark = this.getRemark(this.taskInfo.remark, 3)
				this.filterForm.setFieldsValue(Object.assign({}, this.taskInfo, {remark2: remark}))
			}
		...
		saveForm () {
			...
			this.filterForm.validateFields((error, values) => {
				...
				params.remark = this.addRemark(this.taskInfo.remark, 3, params.remark2)
				params.saveType = '01'
				...
				this.$confirm({
					...
					onOk () {
						return new Promise((resolve, reject) => {
							...
							params.remark = self.addRemark(self.taskInfo.remark, 3, params.remark2)
							params.saveType = '02'
							api.triageApplyInfo(params).then(res => {
								if (res.status === 0) {
