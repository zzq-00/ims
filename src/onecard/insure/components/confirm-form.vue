          ...
          <a-col :span="24">
            <a-form-item label="储值科目">
              <a-select :showSearch="true"  v-decorator="['accountCode', {rules: [{ required: true, message: '储值科目不能为空!' }], initialValue: ''}]">
                <a-select-option v-for="(item, index) in accountCodeList" :value="item.value" :key="index" :disabled="item.disabled" class="spical-disabled-select">{{ item.label }}</a-select-option>
              </a-select>
            </a-form-item>
          </a-col>
        </a-row>
        <a-form-item>
          <a-input type="hidden" v-decorator="['vipStoreApplyId']" />
        </a-form-item>
      </a-form>
      <div slot="footer">
...
export default {
	...
	methods: {
		show (obj) {
			...
			this.$nextTick(() => {
				this.dataForm.setFieldsValue({vipStoreApplyId: obj.vipStoreApplyId})
			})
		},
		loadAccountCodeList () {
			let self = this
		...
		submitForm () {
			...
			this.dataForm.validateFields((error, values) => {
				...
				api.insureAppleConfirm(formData).then(res => {
					if (res.status === 0) {
