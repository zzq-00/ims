...
export default {
	...
	methods: {
		...
		loadList (callback) {
			api.getBatchnoList().then(res => {
				console.log(res, "result")
				this.dataList = []
				res.map(item => {
					if (item && item.batchNo) this.dataList.push({ value: item.batchNo, label: item.batchNo, code: item.batchNo })
				})
				console.log(this.dataList,"batchNo")
				callback && callback()
			})
		},
