...
export default {
	name: 'health-card-product-select',
	props: {
		placeholder: {
			type: String,
			default () {
	...
	methods: {
		...
		loadList (callback) {
			api.getCardProductList({docDefId: this.docDefId}).then(res => {
				console.log(res, "result")
				this.dataList = []
				res.map(item => {
					if (item && item.productname) this.dataList.push({ value: item.productcode, label: item.productname, code: item.productname.split('-')[0]})
				})
