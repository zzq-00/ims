...
export default {
	...
	methods: {
		...
		loadList (callback) {
			api.queryThirdOrgAll().then(res => {
				console.log(res, "result")
				this.dataList = []
				res.data.data.map(item => {
					if (item && item.thirdOrgCode) this.dataList.push({ value: item.thirdOrgCode, label: item.thirdOrgName, code: item.thirdOrgCode})
				})
